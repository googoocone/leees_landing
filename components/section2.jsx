export default function Section2({ visibilityRatio }) {
  // 카드별 애니메이션 지연을 위한 배열 (순서대로 적용)
  const cards = [
    {
      number: "01",
        ...
        {cards.map((card, index) => (
          <div
            ...
            style={{
              ...
              transform: `translate(${visibilityRatio * 100 - 100}px, ${
                (1 - visibilityRatio) * 100
              }px)`,
              transition: `transform 0.5s ease-out ${card.delay}s, opacity 0.5s ease-out`,
          ...
          >
            ...
            <div className="w-full text-justify text-[#534F4B] mt-2 text-[16px] sm:text-[18px]">
              {card.text}
            </div>
          </div>
        ))}
